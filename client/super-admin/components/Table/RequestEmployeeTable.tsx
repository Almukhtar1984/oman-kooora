...
const RequestEmployeeTable = ({ data, searchValue, onEditModal, ...props }: Props) => {
  ...
  const rows = sortedData.map((row, index) => (
    <>
      <tr key={row.id}>
        ...
        <Td>{row.cause}</Td>
        <Td>{row.status}</Td>
        <Td>{dayjs(row.createdTime, "DD/MM/YYYY HH:mm:ss").format("YYYY/MM/DD")}</Td>
    ...
    <Table
      tHead={
        <tr>
          ...
          <Th
            sorted={sortBy === "createdTime"}
            reversed={reverseSortDirection}
            onSort={() => setSorting("createdTime")}
          >
            تاريخ التقديم
          </Th>
          <Th
...
export default RequestEmployeeTable;
