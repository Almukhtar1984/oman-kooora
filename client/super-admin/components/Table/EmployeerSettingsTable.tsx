...
interface RowData {
  id: string;
  employeesSettingName: string;
  EmploymentType: string;
  tolarenceMinutes: string;
  vacationTable: string;
  workSystemTable: string;
  code: string;
}
...
const EmployeerSettingsTable = ({ data, searchValue, ...props }: Props) => {
  ...
  const rows = sortedData.map((row: RowData, index) => (
    <>
      <tr key={row.id}>
        <Td>{row.employeesSettingName}</Td>
        <Td>{row?.EmploymentType}</Td>
        <Td>{row?.tolarenceMinutes} دقيقة</Td>
        <Td>
          <Button
            variant="white"
            color={"orange.5"}
            ...
            onClick={() => {
              useStore.setState({
                tableBreadcrumbs: [
                  {
                    label: "إعدادات الموظفين",

                    onNavigate: (e) => {
                      useStore.setState({ tableBreadcrumbs: [] });
                    },
                  },
                  {
                    navigateTo: "vacationTable",
                    label: row?.employeesSettingName,
                    id: row?.id,
                    onNavigate: (e) => {
                      let newArr = [...(useStore?.getState() as any)?.tableBreadcrumbs];
                      newArr.pop();
                      useStore.setState({ tableBreadcrumbs: newArr });
                    },
                  },
                ],
              });
    ...
    <Table
      tHead={
        <tr>
          <Th
            // colSpan={2}
            sorted={sortBy === "employeesSettingName"}
            reversed={reverseSortDirection}
            onSort={() => setSorting("employeesSettingName")}
          >
            إسم الإعداد
          </Th>
          <Th
          ...
          <Th
            sorted={sortBy === "tolarenceMinutes"}
            reversed={reverseSortDirection}
            onSort={() => setSorting("tolarenceMinutes")}
          >
            زمن السماح
          </Th>
          <Th
            sorted={sortBy === "vacationTable"}
            reversed={reverseSortDirection}
            onSort={() => setSorting("vacationTable")}
          >
            جداول الإجازات السنوية
          </Th>
          <Th
            sorted={sortBy === "workSystemTable"}
            reversed={reverseSortDirection}
            onSort={() => setSorting("workSystemTable")}
          >
            جداول العمل
          </Th>
          <Th
...
export default EmployeerSettingsTable;
