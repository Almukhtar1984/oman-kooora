...
const useAuth = (getCurrentUserLazy: any) => {
    ...
    const checkAuth = useCallback(async (): Promise<boolean> => {
        try {
            // 1. Prefer the access token already in memory / localStorage.
            ...
            if (stillValid) {
                ...
                if (ok) return true;
                // currentUser failed despite a non-expired JWT — try one refresh
                // before giving up (handles backend rotating SECRET_JWT, etc.).
            }

            // 2. Expired or missing token → use the httpOnly refresh cookie.
