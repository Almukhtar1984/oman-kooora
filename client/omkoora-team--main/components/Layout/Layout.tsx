...
const Layout = (props: Props) => {
    ...
    const isAuthRoute = ["/login", "/login/changePassword/[token]", "/login/forGotPassword", "/login/verification/[token]"].includes(router.pathname);
    ...
    return (
        <Box sx={(theme) => ({ backgroundColor: theme.colors.gray[0], minHeight: '100vh' })}>
            ...
            {isAuthRoute
                ...
                : <Box p={"xl"} dir="rtl">{props.children}</Box>
            }
        </Box>
    );
};
