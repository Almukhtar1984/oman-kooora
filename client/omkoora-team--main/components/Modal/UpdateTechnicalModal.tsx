...
const init = {
    occupation: "",
    classification: "",
    membership_date: null as Date | null,
    membership_date_end: null as Date | null,
    paid: false,
    person: {
        first_name: "",
        second_name: "",
...
export const UpdateTechnicalModal = ({id, opened, ...props}: Props) => {
    ...
    useEffect(() => {
        if (dataTechnical && "technicalApparatus" in dataTechnical) {
            const tech = dataTechnical.technicalApparatus;
            form.setValues({
                classification: tech?.classification || "",
                membership_date: tech?.membership_date ? new Date(tech.membership_date) : null,
                membership_date_end: tech?.membership_date_end ? new Date(tech.membership_date_end) : null,
                paid: tech?.paid || false,
                occupation: tech?.occupation || "",
                person: {
                    first_name: tech?.person?.first_name || "",
                    second_name: tech?.person?.second_name || "",
                    third_name: tech?.person?.third_name || "",
                    tribe: tech?.person?.tribe || "",
                    card_number: tech?.person?.card_number || "",
                    date_birth: tech?.person?.date_birth ? new Date(tech.person.date_birth) : null,
                    phone: tech?.person?.phone || ""
                }
            })
            setIdPerson(tech?.person?.id)
        }
    }, [dataTechnical])
    ...
    const onSubmit = (data: any) => {
        ...
        .then(() => {
            notyf?.success("تم تعديل بيانات العضو بنجاح");
            closeModal();
        })
        .catch(reason => {
            console.log(reason)
