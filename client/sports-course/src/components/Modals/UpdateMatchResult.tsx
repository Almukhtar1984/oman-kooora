...
export const UpdateMatchResult = ({data, ...props}: Props) => {
    const {getInputProps, reset, onSubmit, setValues, values, setFieldValue} = useForm({
        initialValues: {
            firstTeamGoal: 0,
            secondTeamGoal: 0,
            hasPenalty: false,
    ...
    useEffect(() => {
        if (data !== null && props.opened) {
            setValues({
                firstTeamGoal: data?.firstTeamGoal,
                secondTeamGoal: data?.secondTeamGoal,
                hasPenalty: !!data?.penalty,
                firstTeamPenalty: data?.penalty?.firstTeamPenalty ?? 0,
                secondTeamPenalty: data?.penalty?.secondTeamPenalty ?? 0,
            })
        }
