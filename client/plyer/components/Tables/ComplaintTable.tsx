...
import {DotsVertical, Check, X, Id, FileCertificate} from "tabler-icons-react";
...
export const ComplaintTable = ({ list, search, setOpenEditModal, setOpenDeleteModal, setSelectedRow }: Props) => {
    ...
    const COLUMNS = [
        ...
        {label: 'الشكوى', renderCell: (item) => `${item?.content}`  },
        {label: 'الملاحظة / سبب الرفض', renderCell: (item) => item?.note },
        {label: 'تاريخ الشكوى', renderCell: (item) => dayjs(item?.createdAt).format("YYYY-MM-DD") },
    ];
