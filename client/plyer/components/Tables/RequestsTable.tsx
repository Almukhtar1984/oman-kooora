...
export const RequestsTable = ({ list, search, setOpenEditModal, setOpenDeleteModal, setSelectedRow }: Props) => {
    ...
    const COLUMNS = [
        ...
        {label: 'الطلب', renderCell: (item) => `${item?.content}`  },
        {label: 'الملاحظة / سبب الرفض', renderCell: (item) => item?.note },
        {label: 'تاريخ الطلب', renderCell: (item) => dayjs(item?.createdAt).format("YYYY-MM-DD") },
    ];
