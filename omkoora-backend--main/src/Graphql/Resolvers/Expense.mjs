...
export const resolvers = {
    Query: {
        ...
        expenseSummary: async (obj, { idClub, idTeam }, context, info) => {
            try {
                let whereClause = {};
                if (idClub) {
                    whereClause.id_club = idClub;
                } else if (idTeam) {
                    whereClause.id_team = idTeam;
                }
                ...
                const profits = expenses
                    .filter(exp => exp.value > 0)
                    .reduce((acc, exp) => acc + exp.value, 0);
                const totalExpenses = expenses
                    .filter(exp => exp.value < 0)
                    .reduce((acc, exp) => acc + Math.abs(exp.value), 0);

                const net = profits - totalExpenses;
    ...
    Mutation: {
        ...
        updateSessionId: async (obj, { id, session_id }, context, info) => {
            try {
                console.error(id)
                ...
                let result = await  await expense.save();
                ...
                console.error(error);
                throw new ApolloError('Failed to update session_id', error);
            }
        },
