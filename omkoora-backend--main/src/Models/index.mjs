...
import participatingPlayersMatchModel    from './ParticipatingPlayersMatch.mjs';
...
import arbitres                 from './Arbitres.mjs';
import authTrace        from "./AuthTrace.mjs";
import senctions        from"./Sanctions.mjs";
import actionLogs        from"./ActionLogs.mjs";
...
const ParticipatingPlayersMatch = participatingPlayersMatchModel(DB, Sequelize);

const Arbitres = arbitres(DB, Sequelize);

const AuthTrace = authTrace(DB, Sequelize);

const Sanction = senctions(DB,Sequelize)
const ActionLog =actionLogs(DB,Sequelize)
const Notification = notification(DB,Sequelize)
const PlayerExternal = playerExternal(DB,Sequelize)
...
// User 1 * League (league admin login — role "4")
...
Match.belongsTo(ParticipatingTeams, {  as: 'firstTeam', foreignKey: { name: 'first_team' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });

// ParticipatingTeams 1 * Match
ParticipatingTeams.hasOne(Match, { foreignKey: { name: 'second_team' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
Match.belongsTo(ParticipatingTeams, {  as: 'secondTeam', foreignKey: { name: 'second_team' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });
...
// Player 1 * MatchCard
Players.hasOne(MatchCard, { foreignKey: { name: 'id_player' },  as: 'playerInfo', onDelete: 'CASCADE', onUpdate: 'CASCADE' })
MatchCard.belongsTo(Players, { foreignKey: { name: 'id_player' },  as: 'playerInfo', onDelete: 'CASCADE', onUpdate: 'CASCADE'  });
...
ParticipatingPlayers.hasOne(ScorerMatch, { foreignKey: { name: 'id_participating_player' }, onUpdate: 'CASCADE' })
ScorerMatch.belongsTo(ParticipatingPlayers, { foreignKey: { name: 'id_participating_player' }, onUpdate: 'CASCADE'  });


// Players 1 * ScorerMatch
//Players.hasOne(ScorerMatch, { foreignKey: { name: 'id_Player' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
//ScorerMatch.belongsTo(Players, { foreignKey: { name: 'id_player' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });

// ParticipatingPlayers 1 * ParticipatingPlayersMatch
ParticipatingPlayers.hasOne(ParticipatingPlayersMatch, { foreignKey: { name: 'id_participating_player' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
ParticipatingPlayersMatch.belongsTo(ParticipatingPlayers, { foreignKey: { name: 'id_participating_player' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });


// Match 1 * ParticipatingPlayersMatch
Match.hasOne(ParticipatingPlayersMatch, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
ParticipatingPlayersMatch.belongsTo(Match, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});


// Match 1 * Arbitres
Match.hasOne(Arbitres, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
Arbitres.belongsTo(Match, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// User 1 * AuthTrace
AuthTrace.belongsTo(User, {foreignKey: { name: 'id_user',allowNull: false },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
User.hasMany(AuthTrace, {foreignKey: { name: 'id_user',allowNull: false },onDelete: 'CASCADE',onUpdate: 'CASCADE'});


// Player 1 * Sanction
Players.hasOne(Sanction, {foreignKey: { name: 'id_player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
Sanction.belongsTo(Players, {foreignKey: { name: 'id_player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// User 1 * ActionLog
User.hasOne(ActionLog, {foreignKey: { name: 'id_user' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
ActionLog.belongsTo(User, {foreignKey: { name: 'id_user' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// Team 1 * ActionLog
Team.hasOne(ActionLog, {foreignKey: { name: 'id_team' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
ActionLog.belongsTo(Team, {foreignKey: { name: 'id_team' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// Club 1 * ActionLog
Club.hasOne(ActionLog, {foreignKey: { name: 'id_club' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
ActionLog.belongsTo(Club, {foreignKey: { name: 'id_club' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// Players 1 * ActionLog
Players.hasOne(ActionLog, {foreignKey: { name: 'id_player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
ActionLog.belongsTo(Players, {foreignKey: { name: 'id_player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});



// Notification 1 * Team
Team.hasOne(Notification, {foreignKey: { name: 'id_team' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
Notification.belongsTo(Team, {foreignKey: { name: 'id_team' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// Notification 1 * Club
Club.hasOne(Notification, {foreignKey: { name: 'id_club' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
Notification.belongsTo(Club, {foreignKey: { name: 'id_club' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

// Notification 1 * Players
Players.hasOne(Notification, {foreignKey: { name: 'id_Player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
Notification.belongsTo(Players, {foreignKey: { name: 'id_Player' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});


// Players 1 * PlayerExternal
PlayerExternal.hasOne(ParticipatingPlayers, { foreignKey: { name: 'id_player_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
ParticipatingPlayers.belongsTo(PlayerExternal, { foreignKey: { name: 'id_player_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });

// Team 1 * Players
Team.hasOne(PlayerExternal, { foreignKey: { name: 'id_team_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
PlayerExternal.belongsTo(Team, { foreignKey: { name: 'id_team_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });

// League 1 * PlayerExternal
League.hasOne(PlayerExternal, { foreignKey: { name: 'id_league_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE' })
PlayerExternal.belongsTo(League, { foreignKey: { name: 'id_league_external' }, onDelete: 'CASCADE', onUpdate: 'CASCADE'  });
// Define the 1:1 relationship
Match.hasOne(Penalty, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});

Penalty.belongsTo(Match, {foreignKey: { name: 'id_match' },onDelete: 'CASCADE',onUpdate: 'CASCADE'});
