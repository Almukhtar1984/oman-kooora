...
يبقي التبويبات الأخرى مسجَّلة دخول لساعات.

**السبب**: كوكي واحد `__tomoh` على نطاق `api.omkooora.com` كان يُرسل من كل
الواجهات. الـ access token JWT صلاحيته 5 ساعات بدون أي قيد على الواجهة، فلا
شيء يمنع إعادة استعماله من تطبيق آخر.

**الحل المُنفَّذ في الباك-إند فقط (لا تغيير في الفرونت)**:

- خريطة `Origin → appKey` (`club`, `team`, `admin`, `player`, …) في
  `src/Config/runtime.mjs` مع helper `refreshCookieName(appKey)`.
- كل تطبيق صار يستخدم كوكي خاص: `__tomoh_club`, `__tomoh_team`, …
- JWT (Access + Refresh) يحمل `aud = appKey`. `AuthMiddleware` و
  `AuthMiddlewareSocket` يرفضان أي توكن مُصدَر لواجهة مختلفة.
- `authenticateUser` / `refreshToken` / `logOut` يقرأون ويكتبون فقط الكوكي
  الخاص بواجهة الـ Origin الحالي.
- `logOut` يحذف أيضاً الكوكي القديم `__tomoh` لتنظيف الجلسات الموروثة.
- عمر access token قُصِّر من 5 ساعات إلى 15 دقيقة افتراضياً
  (`ACCESS_TOKEN_HOURS=0.25`)، حتى يلتقط فاصل التحديث (45 ثانية في الفرونت)
  أي تسجيل خروج من تبويب آخر خلال أقل من دقيقة.
- التوكنات الصادرة قبل التحديث (بدون `aud`) تبقى مقبولة حتى تنتهي تلقائياً
  (transitional).
- حُذف الكود الميت في `authenticateUser` المرتبط بـ `ADMIN_URL` /
  `EMPLOYEE_URL` / `SUPERVISOR_URL` / `CUSTOMER_URL`.

**أثر النشر**: كل المستخدمين الحاليين سيُطلَب منهم تسجيل الدخول من جديد
لأن اسم الكوكي تغيَّر. يلزم تنبيه قبل تشغيل
`bash /root/scripts/deploy.sh api`.

**اختبار محلي مقترح**:
1. شغّل `omkoora-club--main` و `omkoora-team--main` على بورتين مختلفين.
2. سجّل دخول في `club` فقط → افتح `team` → يجب أن يُطلب تسجيل دخول.
3. سجّل دخول في `team` أيضاً → افتح `club` → يجب أن يبقى مسجَّلاً.
4. اضغط خروج من `club` → افتح `team` → يجب أن يبقى `team` مسجَّلاً.
5. خلال أقل من دقيقة من خروج `club`، تبويبات `club` المفتوحة الأخرى يجب
   أن تنتقل تلقائياً لشاشة الدخول.

---

## 2026-04-26

### 59. فحص إضافي لباقي الواجهات

- تم فحص تسجيل الدخول في واجهات `club`, `team`, `super-admin`, `plyer`,
  و`sports-course` ضد الباكند المحلي.
- تم تعديل واجهة `team` حتى تنتقل بعد تسجيل الدخول إلى الداشبورد `/` بدل
  `/members` التي قد تظهر صفحة عدم وجود صلاحية.
- تم إزالة تحذير React في واجهة `plyer` الناتج عن تمرير prop غير مدعوم
  اسمه `styles` إلى عنصر DOM داخل القائمة الجانبية.
- تم التحقق من اختفاء تحذيرات `Maximum update depth` و`Invalid value for prop`
  في واجهة اللاعب أثناء تسجيل الدخول.
- تم تشغيل lint وbuild للتطبيقات المتأثرة، وتشغيل اختبارات الباكند.
